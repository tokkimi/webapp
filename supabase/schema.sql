...
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('resources', 'resources', true, 26214400, array[
...
create policy resources_files_public_read on storage.objects for select using (bucket_id = 'resources');
create policy resources_files_admin_insert on storage.objects for insert to authenticated
  with check (bucket_id = 'resources' and public.is_admin());
create policy resources_files_admin_update on storage.objects for update to authenticated
  using (bucket_id = 'resources' and public.is_admin());
create policy resources_files_admin_delete on storage.objects for delete to authenticated
  using (bucket_id = 'resources' and public.is_admin());
...
create policy page_visits_public_insert on public.page_visits for insert with check (true);
create policy page_visits_admin_read on public.page_visits for select using (public.is_admin());
...
grant all on public.profiles, public.journal_entries, public.mood_entries, public.challenges,
  ...
  public.daily_motivations, public.user_motivations, public.ai_config, public.page_visits to authenticated;
